...
# ============================================================
# KEYS & CONFIG  (set these in Railway / .env file)
# ============================================================
DUONLABS_API_KEY     = os.getenv("DUONLABS_API_KEY", "")
TELEGRAM_BOT_TOKEN   = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHANNEL_ID  = os.getenv("TELEGRAM_CHANNEL_ID", "@SignalPulseLive")
TIKTOK_CLIENT_KEY    = os.getenv("TIKTOK_CLIENT_KEY", "")
TIKTOK_CLIENT_SECRET = os.getenv("TIKTOK_CLIENT_SECRET", "")
DISCORD_BOT_TOKEN    = os.getenv("DISCORD_BOT_TOKEN", "")
DISCORD_CHANNEL_ID   = os.getenv("DISCORD_CHANNEL_ID", "")

GUMROAD_LINK         = os.getenv("GUMROAD_LINK", "https://gumroad.com/l/mlcza3ri")
VIP_PRICE            = "$25/month"
    ...
    if DUONLABS_API_KEY:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[Duon Labs] API returned {response.status_code}, using fallback.")
        except Exception as e:
            print(f"[Duon Labs Error] {e}, using fallback.")

    # Fallback: generate signal if API unavailable or key not set
...
def send_telegram(message: str):
    ...
    try:
        ...
        if r.status_code == 200:
            print("[Telegram] Signal posted successfully")
        else:
...
def send_discord(message: str):
    if not DISCORD_BOT_TOKEN or not DISCORD_CHANNEL_ID:
        print("[Discord] DISCORD_BOT_TOKEN or DISCORD_CHANNEL_ID not set, skipping.")
        return
    try:
        ...
        if r.status_code in [200, 201]:
            print("[Discord] Signal posted successfully")
        else:
...
def post_tiktok(message: str):
    if not TIKTOK_CLIENT_KEY:
        print("[TikTok] TIKTOK_CLIENT_KEY not set, skipping.")
        return
    print(f"[TikTok] Ready to post - Client Key: {TIKTOK_CLIENT_KEY[:8]}...")
    print("[TikTok] OAuth flow required for first-time posting.")
    print("[TikTok] Signal queued for next manual TikTok post.")
